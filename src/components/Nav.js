...
export default function Nav() {
  ...
  const handleLogout = async () => {
    try {
      ...
      // Set an alert message in case of an error during logout
      setAlert({
        isAlert: true,
        severity: 'error',
  ...
  return (
    <React.Fragment>
      <Box sx={{ display: 'flex', alignItems: 'center', textAlign: 'center' }}>
        {!currentUser ? (
          // Button to open the login modal if the user is not logged in
          <Button startIcon={<Lock />} onClick={openLogin}>
            Login
          </Button>
        ) : (
          // Avatar and tooltip for authenticated users
          <Tooltip title="Account settings">
            <IconButton onClick={handleClick} size="small" sx={{ ml: 2 }}>
              ...
              >
                {/* Display user's initial as the avatar content */}
